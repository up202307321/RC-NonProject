...
//Global
LinkLayer conectionParameters_temp;

int Ns = 0;//variavel global  (alternando entre 0 e 1)

int llopen(LinkLayer connectionParameters){
    conectionParameters_temp = connectionParameters;
    ...
    if(connectionParameters.role == LlTx){ //se é transmissor
        ...
        setframe[3] = setframe[1] ^ setframe[2]; //BCC
        setframe[4] = 0x7E; //flag

        alarmCount = 0;
        int UArecebido = 0;
        
        while(alarmCount < conectionParameters_temp.nRetransmissions && UArecebido == 0){
            printf("Set a ser enviado \n");
            ...
            while(alarmEnabled==1 && UArecebido==0){ //esperar por UA
                ...
                if(bytes > 0){//indica que o byte foi lido
                    ...
                    switch(currentstate){
                        ...
                        case C:
                            if (byte == (0x03 ^ 0x07)) {currentstate = XOR;}
                            else {currentstate = Start;}
                            break;
                        case XOR:
    ...
    if(UArecebido==1){
        ...
        printf("UA nao recebido apos n tentativas \n");
        closeSerialPort();
        return -1;
    }
    }
            ...
            if(bytes > 0){
                switch(currentstate){
                        ...
                        case C:
                            ...
                            else if (byte == (0x03 ^ 0x03)) currentstate = XOR;
                            else {currentstate = Start;}
                            break;
                        case XOR:
                            if (byte == 0x7e) { //e o final
...
int llwrite(const unsigned char *buf, int bufSize){
    ...
    //printf( "%d\n"  ,conectionParameters_temp.nRetransmissions);
    while(alarmCount < conectionParameters_temp.nRetransmissions && RR==0 ){
        writeBytesSerialPort(frame, k);
        alarmEnabled = 1;
        alarm(conectionParameters_temp.timeout);
            ...
            if(bytes > 0){//indica que o byte foi lido   
                ...
                switch(currentstate){ 
                    ...
                    case A:
                        ...
                        else if (byte == (Ns == 0 ? 0x81 : 0x01)) //verifca se é uma rejeicao 
...
int llclose(){
    ...
    //transmissor
    /////////////////
    if(conectionParameters_temp.role == LlTx){ //se é transmissor
            ...
            while(alarmEnabled==1 && UArecebido==0){ //esperar por UA
                ...
                if(bytes > 0){//indica que o byte foi lido
                    ...
                    switch(currentstate){
                        ...
                        case C:
                            if (byte == 0x7E) currentstate = FLAG; //volta para o inicio
                            else if (byte == (0x03 ^ 0x0B)) currentstate = XOR;
                            else {currentstate = Start;}
                            break;
                        case XOR:
                            if (byte == 0x7e) { //e o final
    ...
    /////////////////
        ...
        while (uaReceived == 0) {
            ...
            if (bytes > 0) {
                switch (currentstate) {
                    case Start:
                        if (byte == 0x7E) currentstate = FLAG;
                        break;
                    case FLAG:
                        if (byte == 0x03) currentstate = A;
                        else if (byte != 0x7E) currentstate = Start;
                        break;
                    case A:
                        if (byte == 0x07) currentstate = C; // UA
                        else if (byte == 0x7E) currentstate = FLAG;
                        else currentstate = Start;
                        break;
                    case C:
